...
# Constante de version de ton compilateur
__version__ = "1.0.1"

class NebuJS:
    def __init__(self, file_path):
        # Conversion immédiate en chemin absolu réel pour contrer le dossier Temp de Nuitka
    ...
    def _find_file(self, file_path):
        # 1. Test si le chemin fourni est déjà complet/absolu
        p = Path(file_path).resolve()
        if p.exists() and p.is_file(): 
            return p
        
        # 2. Test avec le vrai dossier actuel de la console (os.getcwd())
    ...
    def transpile(self, node):
        # Ignore le code structurel de Tkinter
        if isinstance(node, ast.Assign) and isinstance(node.value, ast.Call):
            func = node.value.func
            if isinstance(func, ast.Attribute) and func.attr == 'Tk':
                return

        # Gestion des appels (print, input, label, mainloop)
        ...
        # Gestion des variables et du input()
    ...
    def run(self):
        try:
            # Lecture forcée via le chemin absolu résolu
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if arg in ("-h", "--help"):
            ...
            sys.exit(0)
        # Gestion des commandes de version
        if arg in ("--version", "-v"):
            print(f"NebuJS Compiler Version {__version__}")
            sys.exit(0)
            
        # Sinon, on considère que c'est le fichier à compiler
        compiler = NebuJS(arg)
        compiler.run()
    else:
        print("Usage:")
